// ==UserScript==
// @name            Whitecape
// @description     A proof of concept of a chess helper
// @version         1.0
// @namespace       bcattaneo.Whitecape
// @grant           none
// @include         https://lichess.org/*
// @updateURL       https://www.cattaneo.uy/whitecape/whitecape.meta.js
// @downloadURL     https://www.cattaneo.uy/whitecape/whitecape.user.js
// @icon            data:image/gif;base64,
// ==/UserScript==

const version = "1.0";
const stepsTag = "u8t";
const whitecapeUrl = "http://127.0.0.1:5000/best-move";
const checkTimeout = 500;
const debugEnabled = true;
const yourTurnClass = "rclock-turn__text";
let steps = [];
let blackPlayer = false;

window.addEventListener("load", main);

function showMessage(message) {
  // document.getElementsByClassName(yourTurnClass)[0].innerHTML = message;
}

function getMoveCoords(move) {
  const start = "a".charCodeAt(0);
  return [
    -3.5 + 1 * (move[0].charCodeAt(0) - start),
    3.5 - 1 * (parseInt(move[1]) - 1),
  ];
}

function buildCircles(move) {
  // TODO
  // debug(`Building circles for move ${move}`);
  // console.log(move);
  // const from = move.slice(0, 2);
  // const fromCoords = getMoveCoords(from);
  // const to = move.slice(2, 4);
  // const toCoords = getMoveCoords(to);
  // console.log(from);
  // console.log(fromCoords);
  // console.log(to);
  // console.log(toCoords);
  // document.getElementsByTagName("g")[0].innerHTML
  // const circles = `<circle stroke="#FFFFFFF" stroke-width="0.0625" fill="none" opacity="1" cx="${fromCoords[0]}" cy="${fromCoords[1]}" r="0.46875" cgHash="505.6000061035156,505.6000061035156,${from},white" />`;
  // c.setAttributeNS(null, "stroke", "#FFFFFFF");
  // c.setAttributeNS(null, "stroke-width", "0.0625");
  // c.setAttributeNS(null, "fill", "none");
  // c.setAttributeNS(null, "opacity", "1");
  // c.setAttributeNS(null, "cx", "-3.5");
  // c.setAttributeNS(null, "cy", "-3.5");
  // c.setAttributeNS(null, "r", "0.46875");
  // c.setAttributeNS(null, "cgHash", "505.6000061035156,505.6000061035156,a8,white");
  //   <circle
  //   stroke="#FFFFFFF"
  //   stroke-width="0.0625"
  //   fill="none"
  //   opacity="1"
  //   cx="${toCoords[0]}"
  //   cy="${toCoords[1]}"
  //   r="0.46875"
  //   cgHash="505.6000061035156,505.6000061035156,${to},white"
  // </circle>
  // document.getElementsByTagName("g")[0].innerHTML = circles;
  // console.log(circles);
}

function debug(message) {
  if (debugEnabled) console.log(`[whitecape] ${JSON.stringify(message)}`);
}

function loop() {
  const newSteps = getSanSteps();

  if (JSON.stringify(newSteps) !== JSON.stringify(steps)) {
    debug("Steps changed...");
    debug(newSteps);
    steps = newSteps;

    if (
      (blackPlayer && newSteps.length % 2 != 0) ||
      (!blackPlayer && newSteps.length % 2 == 0)
    ) {
      debug(`Getting move for ${blackPlayer ? "black" : "white"}`);
      showMessage("Getting best move...");
      getMove(newSteps);
    }
  }

  setTimeout(loop, checkTimeout);
}

function main() {
  // TODO: only run this script on specific urls (matches, puzzles and so on)
  debug(`whitecape ${version} loaded`);
  blackPlayer = isBlack();
  loop();
}

function isBlack() {
  return Array.from(
    document.getElementsByTagName("coords")[0].classList
  ).includes("black");
}

function getMove(steps) {
  fetch(whitecapeUrl, {
    method: "POST",
    body: JSON.stringify(steps),
    headers: {
      "Content-Type": "application/json",
    },
  }).then((results) => {
    results.json().then((x) => {
      debug(x);
      debug(`Best move is ${x.data}`);
      // showMessage(`Best move is <b>${x.data}</b>`);
      // buildCircles(x.data);
    });
  });
}

function getSanSteps() {
  return Array.from(document.getElementsByTagName(stepsTag)).map(
    (n) => n.textContent
  );
}
